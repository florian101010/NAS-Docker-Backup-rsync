...
stop_all_docker_stacks() {
    ...
    if [[ $PARALLEL_JOBS -gt 1 ]]; then
        ...
        export GREEN RED YELLOW BLUE CYAN NC  # Color variables for parallel mode
        export -f process_docker_output format_container_status log_write log_message

        # Parallel stopping with xargs (robust parameter passing)
...
start_all_docker_stacks() {
    ...
    if [[ $PARALLEL_JOBS -gt 1 ]]; then
        ...
        # Parallel starting with xargs (robust parameter passing)
        printf '%s\0' "${ALL_STACKS[@]}" | xargs -0 -r -P "$PARALLEL_JOBS" -I {} \
          bash -c '
            stacks_dir="$1"
            stack_name="$2"
            ...
            if [[ -f "$stack_dir/docker-compose.yml" ]]; then
                echo "  → Starting stack (parallel): $stack_name"
                if timeout "$timeout_start" bash -c "cd \"$stack_dir\" && $sudo_cmd docker compose up -d" 2>&1 | process_docker_output; then
                    echo "    ${GREEN}✅ Stack ${GREEN}$stack_name ${GREEN}started${NC}"
                    touch "$temp_dir/$stack_name.success"
                else
                    echo "    ${RED}❌ Error starting: ${GREEN}$stack_name${NC}"
                    touch "$temp_dir/$stack_name.failed"
                fi
            fi
...
perform_consolidated_health_check() {
    ...
    echo -e "${BLUE}🔍 Checking status of ${#ALL_STACKS[@]} stacks...${NC}"
    
    # Robust approach: Use docker compose ps without temporary files
    for stack_name in "${ALL_STACKS[@]}"; do
        local stack_dir="$STACKS_DIR/$stack_name"
        
        if [[ ! -f "$stack_dir/docker-compose.yml" ]]; then
            log_message "WARN" "Health check: docker-compose.yml not found for stack: $stack_name"
            continue
        fi
        
        # Robust container counting without temporary files
        ...
        # Count all containers (with increased timeout)
        ...
        if all_containers_output=$(timeout 15 bash -c "cd '$stack_dir' && $SUDO_CMD docker compose ps -q 2>/dev/null"); then
            ...
            log_message "WARN" "Health check: Timeout getting containers for stack: $stack_name"
            continue
        fi
        
        # Count running containers (robust approach without --status flag)
        ...
        ((healthy_containers += running_count))
        ((unhealthy_containers += (total_count - running_count)))
    ...
    echo ""
    echo -e "${CYAN}=== CONTAINER STATUS ===${NC}"
    echo -e "${GREEN}✅ Running containers: $healthy_containers${NC} | ${RED}❌ Stopped: $unhealthy_containers${NC} | ${BLUE}📊 Total: $total_containers${NC}"
    echo -e "${GREEN}✅ Fully running stacks: $healthy_stacks${NC} | ${YELLOW}⚠️ With stopped containers: $unhealthy_stacks${NC} | ${BLUE}📊 Total: ${#ALL_STACKS[@]}${NC}"
    
    if [[ $unhealthy_stacks -eq 0 && $unhealthy_containers -eq 0 ]]; then
        echo -e "${GREEN}🎉 All containers are started and running${NC}"
        log_message "INFO" "Health check: All $total_containers containers in $healthy_stacks stacks are running"
    else
        echo -e "${YELLOW}⚠️ Some containers are stopped - check with 'docker ps' for details${NC}"
        log_message "WARN" "Health check: $unhealthy_containers stopped containers in $unhealthy_stacks stacks"
    fi
...
perform_backup() {
    ...
    if [[ "$PRESERVE_ACL" == true ]]; then
        ...
        if command -v setfacl >/dev/null 2>&1; then
            ...
            if $SUDO_CMD touch "$acl_test_file" 2>/dev/null && $SUDO_CMD setfacl -m u:$(whoami):rw "$acl_test_file" 2>/dev/null; then
                # Test and add ACL/xattr flags individually
                if test_rsync_flag "-A"; then
                    RSYNC_FLAGS+=" -A"
                    log_message "INFO" "ACL support added"
                fi
                if test_rsync_flag "-X"; then
                    RSYNC_FLAGS+=" -X"
                    log_message "INFO" "Extended attributes support added"
                fi
                if test_rsync_flag "-H"; then
                    RSYNC_FLAGS+=" -H"
                    log_message "INFO" "Hardlink support added"
                fi
    ...
    if execute_rsync_backup "$BACKUP_SOURCE" "$BACKUP_DEST" "$RSYNC_FLAGS"; then
        ...
        log_message "INFO" "Fallback: Using minimal flags: $RSYNC_FLAGS"
        ...
        if execute_rsync_backup "$BACKUP_SOURCE" "$BACKUP_DEST" "$RSYNC_FLAGS"; then
            ...
            RSYNC_FLAGS="-a --delete"
            log_message "INFO" "Basic fallback: Using only: $RSYNC_FLAGS"
...
# Function for compact backup statistics at the end
show_backup_summary() {
    if [[ "$SKIP_BACKUP" == true ]]; then
        return 0
    fi
    ...
    echo ""
    echo -e "${BLUE}🔍 Backup Overview - File sizes and counts:${NC}"
    
    # Check if backup directory exists
    if [[ ! -d "$BACKUP_DEST" ]]; then
        echo -e "${YELLOW}⚠️ Backup directory not found for statistics${NC}"
        return 0
    fi
    
    # Calculate sizes and file counts (with timeout for large directories)
    local source_size=""
    local backup_size=""
    local source_files=""
    local source_dirs=""
    ...
    # Use timeout to avoid hanging on very large directories
    ...
    # File and directory counts (with timeout)
    ...
    if [[ -n "$source_size" && -n "$backup_size" ]]; then
        ...
        echo -e "${YELLOW}⚠️ Size calculation not available${NC}"
    fi
    ...
    if [[ -n "$source_files" && -n "$source_dirs" && -n "$backup_files" && -n "$backup_dirs" ]]; then
        ...
        echo -e "${YELLOW}⚠️ File count calculation not available${NC}"
    fi
}
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    # Show compact backup statistics
    show_backup_summary

    # Deactivate EXIT trap BEFORE health check (prevents double container start)
    trap - EXIT

    # Compact health check (always run when not DRY_RUN, regardless of container start status)
    if [[ "$DRY_RUN" == false ]]; then
        # Ensure ALL_STACKS is populated before health check
        discover_docker_stacks
        perform_consolidated_health_check
    fi
...
# Single completion log (trap already deactivated earlier)
